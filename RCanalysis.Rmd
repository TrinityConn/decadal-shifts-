...
plot_dat <- out %>%
  mutate(
    species = toupper(str_c(str_sub(host_genus, 1, 1), str_sub(host_species, 1, 3), sep = ""))
  ) %>%
  ...
  mutate(
    Clade = factor(Clade, levels = c("A", "B", "C", "D", "G"))
  )

ggplot(plot_dat, aes(x = factor(sample_id), y = Prop, fill = Clade)) +
  geom_col(width = 0.95) +
  facet_grid(
    species ~ era,
    scales = "free_x",
...
out2 <- out %>%
  mutate(
    ...
    Species     = code_from_species(Species_raw)
  )
...
compute_pair <- function(dat, era1, era2, min_n = 4, depth_std = TRUE, n_bins = 6) {
...
res_A <- map_dfr(pairs, ~compute_pair(dat, .x[1], .x[2], depth_std = TRUE)) %>%
  mutate(analysis = "A_depth_std")

# Sensitivity: no depth standardization
res_no <- map_dfr(pairs, ~compute_pair(dat, .x[1], .x[2], depth_std = FALSE)) %>%
...
#new?
```{r}
# count number of samples of each species in each era in each subregion
sppcounts <- dat %>%
  group_by(Species, subregion, era) %>%
  summarize(n = n())

ggplot(sppcounts, aes(x = era, y = n)) +
  geom_col() +
  facet_grid(subregion ~ Species, scales = "free_y")
...
View(spptots)
sum(spptots$n)
n_distinct(spptots$Species)

# ============================================================
# START FROM YOUR EXISTING dat OBJECT
# Assumes dat already contains:
# Species, era, subregion, depth, and clade columns A,B,C,D,G
# ============================================================
...
mean_comp <- function(df, w = NULL, clades) {
  ...
  if (is.null(w)) w <- rep(1, nrow(df))
  if (sum(w, na.rm = TRUE) == 0) w <- rep(1, nrow(df))
  w <- w / sum(w, na.rm = TRUE)

  p <- colSums(X * w, na.rm = TRUE)

  if (sum(p) == 0) {
    p <- rep(NA_real_, length(clades))
    names(p) <- clades
    return(p)
  }
...
make_depth_bins <- function(df, n_bins = 6) {
  qs <- seq(0, 1, length.out = n_bins + 1)
  edges <- quantile(df$depth, probs = qs, na.rm = TRUE)
  edges <- unique(as.numeric(edges))

  if (length(edges) <= 2) return(NULL)

  cut(df$depth, breaks = edges, include.lowest = TRUE)
}

get_depth_weights <- function(subg, target_bins) {
  target <- tibble(depth_bin = target_bins) %>%
    count(depth_bin, name = "n_target") %>%
    mutate(p_target = n_target / sum(n_target))

  src <- subg %>%
    count(depth_bin, name = "n_src") %>%
    mutate(p_src = n_src / sum(n_src))

  wt_tbl <- target %>%
    left_join(src, by = "depth_bin") %>%
    mutate(
      p_src = replace_na(p_src, 0),
      w_bin = p_target / ifelse(p_src == 0, NA_real_, p_src),
      w_bin = ifelse(is.na(w_bin) | is.infinite(w_bin), 0, w_bin)
    )

  subg %>%
    left_join(wt_tbl %>% dplyr::select(depth_bin, w_bin), by = "depth_bin") %>%
    mutate(w_bin = replace_na(w_bin, 0)) %>%
    pull(w_bin)
}

# ============================================================
# CORE FUNCTION
# Uses different standardization approaches by subregion:
# - Upper Keys   -> depth standardized
# - Dry Tortugas -> not depth standardized
# - all others   -> excluded unless you choose to keep them
# ============================================================

compute_subregion_outputs <- function(dat,
                                      eras = c("2001-03", "2019-21", "2024"),
                                      clades = c("A", "B", "C", "D", "G"),
                                      min_n = 4,
                                      n_bins = 6) {

  dat_use <- dat %>%
    filter(subregion %in% c("Upper Keys", "Dry Tortugas")) %>%
    filter(era %in% eras) %>%
    mutate(
      era = factor(era, levels = eras, ordered = TRUE)
    )
  ...
  pair_df <- tibble(
    era1 = c("2001-03", "2019-21", "2001-03"),
    era2 = c("2019-21", "2024", "2024")
  )

  out_comp <- vector("list", length(split_dat))
  out_dist <- vector("list", length(split_dat))

  for (i in seq_along(split_dat)) {
    g <- split_dat[[i]]
    key <- group_keys[i, , drop = FALSE]

    this_subregion <- unique(g$subregion)
    use_depth_std <- this_subregion == "Upper Keys"

    # require at least 2 eras with enough observations
    era_counts <- g %>% count(era, name = "n")
    if (sum(era_counts$n >= min_n) < 2) next
    ...
    if (use_depth_std) {
      ...
      if (nrow(g_depth) == 0) next

      g_depth <- g_depth %>%
        mutate(depth_bin = make_depth_bins(., n_bins = n_bins))

      if (all(is.na(g_depth$depth_bin))) next

      target_bins <- g_depth$depth_bin

      comp_one <- map_dfr(eras, function(e) {
        subg <- g_depth %>% filter(era == e)
        ...
        if (nrow(subg) < min_n) {
          tibble(
            ...
            n = nrow(subg),
            depth_std = TRUE,
            !!!setNames(as.list(rep(NA_real_, length(clades))), clades)
          )
        } else {
          w <- get_depth_weights(subg, target_bins = target_bins)
          p <- mean_comp(subg, w = w, clades = clades)
      ...
      dist_one <- map_dfr(seq_len(nrow(pair_df)), function(j) {
        e1 <- pair_df$era1[j]
        e2 <- pair_df$era2[j]

        g1 <- g_depth %>% filter(era == e1)
        g2 <- g_depth %>% filter(era == e2)

        if (nrow(g1) < min_n || nrow(g2) < min_n) {
          tibble(
            era1 = e1,
            era2 = e2,
            n1 = nrow(g1),
          ...
          w1 <- get_depth_weights(g1, target_bins = target_bins)
          w2 <- get_depth_weights(g2, target_bins = target_bins)

          p1 <- mean_comp(g1, w = w1, clades = clades)
          p2 <- mean_comp(g2, w = w2, clades = clades)
      ...
      comp_one <- map_dfr(eras, function(e) {
        ...
        if (nrow(subg) < min_n) {
          ...
          p <- mean_comp(subg, clades = clades)
    ...
    out_comp[[i]] <- bind_cols(key[rep(1, nrow(comp_one)), , drop = FALSE], comp_one)
    out_dist[[i]] <- bind_cols(key[rep(1, nrow(dist_one)), , drop = FALSE], dist_one)
  }

  list(
    composition = bind_rows(out_comp),
    distances   = bind_rows(out_dist)
  )
}
...
# RUN
# ============================================================
res_subregion <- compute_subregion_outputs(
  dat = dat2,
  eras = c("2001-03", "2019-21", "2024"),
  clades = clades,
  min_n = 4,
...
comp_subregion <- res_subregion$composition
dist_subregion <- res_subregion$distances

# ============================================================
# LONG FORMAT FOR PLOTTING
# ============================================================
comp_subregion_long <- comp_subregion %>%
  pivot_longer(
    cols = all_of(clades),
    names_to = "Clade",
    values_to = "Prop"
  ...
  mutate(
    Clade = factor(Clade, levels = clades, labels = clade_labs[clades]),
    era = factor(era, levels = c("2001-03", "2019-21", "2024"), ordered = TRUE)
  )

dist_subregion <- dist_subregion %>%
  mutate(
    contrast = factor(
      paste0(era1, " \u2192 ", era2),
      levels = c("2001-03 → 2019-21", "2019-21 → 2024", "2001-03 → 2024")
    )
  )
...
# ---- Composition plot ----
p_comp_subregion <- ggplot(
  comp_subregion_long,
  aes(x = era, y = Prop, fill = Clade)
) +
...
p_dist_subregion <- ggplot(
  dist_subregion,
  aes(x = contrast, y = Hellinger)
) +
  geom_col(width = 0.7) +
  facet_grid(subregion ~ Species) +
  labs(
...
# ============================================================
# OPTIONAL: ADD LABEL FOR STANDARDIZATION METHOD
# ============================================================
comp_subregion_long <- comp_subregion_long %>%
  mutate(method = ifelse(depth_std, "Depth-standardized", "Not depth-standardized"))

dist_subregion <- dist_subregion %>%
  mutate(method = ifelse(depth_std, "Depth-standardized", "Not depth-standardized"))

# Example alternate distance plot with method in strip label
p_dist_subregion2 <- ggplot(
  dist_subregion,
  aes(x = contrast, y = Hellinger)
) +
  geom_col(width = 0.7) +
  facet_grid(subregion + method ~ Species) +
  labs(
    x = NULL,
    y = "Hellinger distance"
  ) +
...
# EXPORT
# ============================================================
write_csv(comp_subregion, "depthmix_composition_by_subregion_species.csv")
write_csv(dist_subregion, "depthmix_hellinger_by_subregion_species.csv")
...
comp_subregion_long %>%
  filter(n > 0, !is.na(Prop)) %>%
  ggplot(aes(x = era, y = Prop)) +
...
comp_subregion_long %>%
  filter(Clade == "Clade D", n > 0, !is.na(Prop)) %>%
  group_by(subregion, era) %>%
  summarize(D75 = n_distinct(Species[Prop > 0.75]),
            D2575 = n_distinct(Species[Prop > 0.25 & Prop < 0.75]),
            D25 = n_distinct(Species[Prop < 0.25]))

comp_subregion_long %>%
  filter(n > 0, subregion == "Upper Keys", era == "2001-03", Clade == "Clade D") 
...
comp_subregion_long %>%
  filter(Clade == "Clade D", n > 0, !is.na(Prop), !Species %in% c("DSTO", "UNKN")) %>%
  group_by(subregion, Species, era) %>%
  summarize(Prop = mean(Prop), .groups = "drop") %>%
  arrange(subregion, Species, era) %>%
  group_by(subregion, Species) %>%
  summarize(
    delta = last(Prop) - first(Prop),
    direction = case_when(
      delta >  0 ~ "increase",
      delta <  0 ~ "decrease",
      TRUE       ~ "no change"
...
comp_subregion_long %>%
  ...
  group_by(subregion, Species) %>%
  summarize(
    Prop_start = first(Prop),
    Prop_end   = last(Prop),
    delta      = Prop_end - Prop_start,
    .groups = "drop"
  ) %>%
  ggplot(aes(x = Prop_start, y = Prop_end)) + 
    geom_point(aes(color = subregion)) +
    geom_text(aes(label = Species, color = subregion)) +
...
saveRDS(dist_subregion, file = "dist_subregion.rds")
dist_subregion
rich <- read_csv("data/rossSpeciesShuffCoef.csv") %>%
  mutate(Species = toupper(Species))

alldist <- left_join(dist_subregion, rich, by = "Species")

alldist %>%
  filter(contrast == "2019-21 → 2024") %>%
  ggplot(aes(y = JSD, x = mean_shuffCoef)) +
  geom_point() +
  geom_text(aes(label = Species)) +
  geom_smooth(method = "lm") +
  ggpubr::stat_cor()

saveRDS(comp_subregion, file = "comp_subregion.rds")
allcomp <- left_join(comp_subregion, rich, by = "Species")

allcomp %>%
  filter(era == "2024") %>%
  ggplot(aes(x = mean_propD, y = D)) +
  geom_point() +
  geom_text(aes(label = Species)) +
  geom_smooth(method = "lm") +
  ggpubr::stat_cor()
